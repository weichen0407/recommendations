"""Stage two: turn validated briefs into task prompts without reclassifying their tags."""
...
SUMMARY_RULES = """You write research execution instructions for Eureka from approved content briefs.
...
or return a report, answer or factual conclusion. The node is named generate_summary, but its
output is research instructions for future execution, not a summary of an existing article.
...
Return JSON only: {"summaries": [{"brief_id": "...", "content_category": "...",
...
def summary_response_schema(briefs: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    properties = {
...
def validate_summaries(payload: Any, briefs: list[dict[str, Any]]) -> list[str]:
    if not isinstance(payload, dict) or set(payload) != {"summaries"}:
        return ["Root must contain only summaries."]
    items = payload["summaries"]
    if not isinstance(items, list) or len(items) != len(briefs):
        return ["Return exactly one summary per brief."]
    ...
    for i, item in enumerate(items):
        if not isinstance(item, dict) or set(item) != {
            ...
            errors.append(f"summaries[{i}]: unexpected or missing fields")
            continue
        key = item["brief_id"]
        if not isinstance(key, str) or key not in expected or key in seen:
            errors.append(f"summaries[{i}]: unknown or duplicate brief_id")
        else:
            seen.add(key)
        if item["content_category"] not in content_category_values():
            errors.append(f"summaries[{i}]: content_category must be an allowed enum")
        instructions = item["research_instructions"]
        if not isinstance(instructions, str) or not 20 <= len(instructions.strip()) <= 12000:
            errors.append(
                f"summaries[{i}]: research_instructions must contain 20..12000 characters"
            )
    if seen != expected:
...
def generate_task_specs(
    ...
    messages = [
        {
            ...
            + "\nResponse schema:\n"
            + json.dumps(summary_response_schema(briefs)),
        },
        {
            "role": "user",
            "content": json.dumps(
    ...
    for attempt in range(2):
        try:
            ...
            raise GenerationError(
                "generate_summary", [f"LLM request failed ({type(exc).__name__})."]
            ) from None
        try:
            payload = parse_brief_response(raw)
            errors = validate_summaries(payload, briefs)
        except (ValueError, RecursionError):
            errors = ["Response must be valid JSON with unique keys."]
        if not errors:
            by_id = {item["brief_id"]: item for item in payload["summaries"]}
            return [
    ...
    raise GenerationError("generate_summary", errors)


def build_task_spec(brief, summary, language, output_format):
    ...
    prompt = (
        ...
        + summary["research_instructions"].strip()
    ...
    return {
        ...
        "research_instructions": summary["research_instructions"],
        "content_category": summary["content_category"],
        "language": language,
